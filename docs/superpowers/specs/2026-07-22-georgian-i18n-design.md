...
2. **main.js `setLang`/фолбэк** — ПРОВЕРЕНО В КОДЕ (уточнение): `setLang('ge')` НЕ обнуляет текст (`void 0!==dataset[e]` пропускает отсутствующий `data-ge`), только ставит валюту gel. Реальные правки: (а) добавить ветку редиректа `ge` (savedLang=ge на не-/ge/ → `location.replace('/ge'+path)`) — сейчас её нет; (б) EN-фолбэк динамики: заменить 9 инлайн `"en"===documentElement.lang` → `"ru"!==documentElement.lang`. Доказуемо RU/EN-безопасно: на RU (`ru`) и EN (`en`) поведение идентично, меняется только `/ge/` (`ka`): RU→EN. Затрагивает лишь модалку тура + квиз + строки формы (карточки туров статичны).
...
## Tasks (пошагово) + строгая оценка

Легенда: Риск (прод-влияние) · Увер. (шанс пройти с 1-й попытки) · Режим отказа.

### Ф0 — изоляция ✅ СДЕЛАНО
- [x] T0.1 secret-check + sync-коммит 478 файлов. Риск:низк · 100% · —
- [x] T0.2 ветка feature/georgian-lang + коммит спеки. Риск:низк · 100% · —

### Ф1 — ассеты ✅ СДЕЛАНО
- [x] T1.1 Noto Sans Georgian woff2 → /fonts/. Риск:нет · 100% (валиден) · —
- [x] T1.2 css/ge.css (@font-face + nowrap-оверрайды). Риск:нет · 95% · оверрайд может не покрыть все узкие места → ловит Ф6 QA

### Ф2 — общие файлы 🔴 (главный прод-риск)
- [ ] T2.1 main.js: ветка редиректа `ge`. Accept: savedLang=ge на не-/ge/ → replace на /ge/. Verify: регресс. Файлы: main.js. Риск:🔴 · 90% · сломать тернарник редиректа → RU/EN редирект ломается
- [ ] T2.2 main.js: 9× `"en"===lang`→`"ru"!==lang`. Accept: каждый — реально язык-тоггл (проверить поимённо). Verify: diff RU/EN идентичен. Риск:🔴 · 85% · один из 9 — не язык-тоггл → ложная инверсия
- [ ] T2.3 main.js: fetch отзывов `lang=ge` на /ge/. Риск:средн · 90% · —
- [ ] T2.4 reviews.js: REVIEWS_GE + isGe + `Vary`. Verify: /ge/ отдаёт GE. Риск:средн · 90% · CDN-кеш всё равно склеит если Vary неполный
- [ ] T2.5 middleware.js: `ka`→/ge/, /ge/ в SKIP+matcher. Verify: curl Accept-Language. Риск:🔴 · 88% · matcher-regex опечатка → middleware не грузится, весь сайт без язык-редиректа
- [ ] T2.6 РЕГРЕСС-ТЕСТ RU/EN. Accept: карточки/квиз/отзывы/редиректы RU/EN не изменились. Gate перед Ф3. Риск:— · критичный гейт

### Ф3 — трансформер (пер-типовый)
- [ ] T3.1 gen-ge-core.js: canonical→/ge/, hreflang-квадры, og+alternates, lang=ka, schema @id/url/inLanguage/knowsLanguage/breadcrumb, переключатель, preload. Риск:нет(не деплой) · 70% · хрупкость якорей на разных типах
- [ ] T3.2 пер-тип: патч блог-скрипта (88 файлов) + аномалии. Риск:нет · 75% · регекс не матчит вариацию
- [ ] T3.3 пер-тип: главная (данные main.js). Риск:нет · 70% · —
- [ ] T3.4 predeploy-check.js +GE-проверки (canonical-self, lang=ka, шрифт, нет EN-прозы). Риск:нет · 85% · —
- [ ] T3.5 прогон на 1 файле каждого типа + валидация. Gate. · 65% · всплывут пер-типовые баги (ожидаемо)

### Ф4 — перевод (качество без носителя)
- [ ] T4.1 грунтинг-корпус .ge + глоссарий. Риск:нет · 80% · мало native-источников
- [ ] T4.2 субагент EN→KA (только текст-ноды, 6 стр). Риск:нет · 75% · заденет структуру/слаги/schema-ключи
- [ ] T4.3 критик-субагент (беглость≥4/5). Риск:нет · 70% · критик не носитель — пропустит тонкие кальки
- [ ] T4.4 обратный перевод KA→RU, сверка смысла. Gate. · 80% · —

### Ф5 — двойники + sitemap
- [ ] T5.1 hreflang ka + GE-кнопка на 6 RU + 6 EN двойниках (правило: только при наличии GE). Риск:низк · 85% · правка чужих RU/EN файлов
- [ ] T5.2 sitemap: 6 /ge/ url + xhtml:link ka в ru/en записи. Риск:низк · 85% · рассинхрон HTML↔sitemap (былая грабля)
- [ ] T5.3 seo-hreflang реципрокность. Gate. · 90% · —

### Ф6 — локальная QA + деплой-гейт
- [ ] T6.1 predeploy-check всех пилотных. · 90%
- [ ] T6.2 chrome-devtools рендер 375/768/1920. · 80% · вязь обрежет неучтённый элемент
- [ ] T6.3 переключатель RU↔EN↔GE во все стороны. · 85%
- [ ] T6.4 curl canonical/hreflang. Gate=GO/NO-GO. · 90%
